...
def get_chart_data(data, filters):
	"""نمودار مقایسه قیمت‌ها"""
	if not data:
		return None
	
	# محدود کردن به 20 کالای اول برای خوانایی نمودار
	chart_data = data[:20]
	
	labels = [row.get("item_name", "")[:30] for row in chart_data]  # محدود به 30 کاراکتر
	price_list_1 = [flt(row.get("price_list_1_rate", 0)) for row in chart_data]
	price_list_2 = [flt(row.get("price_list_2_rate", 0)) for row in chart_data]
	total_costs = [flt(row.get("total_cost", 0)) for row in chart_data]
	...
	return {
		"data": {
			...
			"datasets": [
				{
					"name": filters.get("price_list_1", "لیست اول"),
					"values": price_list_1
				},
				{
					"name": filters.get("price_list_2", "لیست دوم"),
					"values": price_list_2
				},
				{
					"name": "بهای تمام شده",
